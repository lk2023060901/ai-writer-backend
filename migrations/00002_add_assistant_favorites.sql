...
CREATE INDEX IF NOT EXISTS idx_favorites_user_id ON assistant_favorites(user_id, sort_order ASC);

-- 索引：根据 assistant_id 查询有多少用户收藏了该智能体
CREATE INDEX IF NOT EXISTS idx_favorites_assistant_id ON assistant_favorites(assistant_id);

-- 索引：按添加时间查询
CREATE INDEX IF NOT EXISTS idx_favorites_created_at ON assistant_favorites(created_at DESC);
